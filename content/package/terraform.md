+++
draft = false
title = "terraform 1.5.7-1"
version = "1.5.7-1"
description = "HashiCorp tool for building and updating infrastructure as code idempotently"
date = "2023-09-11T19:34:50"
aliases = "/packages/220010"
categories = ['devel-extra']
upstreamurl = "https://www.terraform.io/"
arch = "x86_64"
size = "14239464"
usize = "65890716"
sha1sum = "8aca562ef4748bf39663a68dd9253db0d685772e"
depends = "['glibc']"
+++
HashiCorp tool for building and updating infrastructure as code idempotently{{< spoiler text="show files" >}}* /usr/bin/terraform
* /usr/share/doc/terraform-1.5.7/LICENSE
* /usr/share/doc/terraform-1.5.7/README.md
{{< /spoiler >}}