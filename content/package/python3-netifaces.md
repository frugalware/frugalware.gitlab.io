+++
draft = false
title = "python3-netifaces 0.11.0-6"
version = "0.11.0-6"
description = "Portable module to access network interface information in Python3."
date = "2023-10-11T18:39:31"
aliases = "/packages/220271"
categories = ['lib-extra']
upstreamurl = "http://alastairs-place.net/projects/netifaces"
arch = "x86_64"
size = "11460"
usize = "40757"
sha1sum = "9a490815059e85a4686cbc63ab96d55b761ff41f"
depends = "['python3>=3.11']"
reverse_depends = "['python3-zeroconf']"
+++
Portable module to access network interface information in Python3.{{< files text="show files" >}}* /usr/lib/python3.12/site-packages/netifaces-0.11.0-py3.12.egg-info/dependency_links.txt
* /usr/lib/python3.12/site-packages/netifaces-0.11.0-py3.12.egg-info/PKG-INFO
* /usr/lib/python3.12/site-packages/netifaces-0.11.0-py3.12.egg-info/SOURCES.txt
* /usr/lib/python3.12/site-packages/netifaces-0.11.0-py3.12.egg-info/top_level.txt
* /usr/lib/python3.12/site-packages/netifaces-0.11.0-py3.12.egg-info/zip-safe
* /usr/lib/python3.12/site-packages/netifaces.cpython-312-x86_64-linux-gnu.so
* /usr/share/doc/python3-netifaces-0.11.0/LICENSE
* /usr/share/doc/python3-netifaces-0.11.0/README.rst
{{< /files >}}