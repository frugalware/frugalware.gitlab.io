+++
draft = false
title = "luminance-hdr 2.6.1.1-10"
version = "2.6.1.1-10"
date = "2023-06-15T09:39:23"
categories = ['xapps-extra']
upstreamurl = "https://sourceforge.net/projects/qtpfsgui"
arch = "x86_64"
size = "4908304"
usize = "10834520"
sha1sum = "9316ff047cdf58e12546cc62435a11bde24d69d9"
depends = "['exiv2>=0.27', 'openexr>=3.1.0', 'libgomp>=6.2.1-5', 'fftw', 'gsl>=2.7.1', 'libraw>=0.21.1', 'lcms2>=2.8-2', 'libboost>=1.82.0', 'qt5-svg>=5.15.10', 'qt5-webengine', 'eigen']"
+++
An open source graphical user interface application that aims to provide a workflow for HDR imaging