+++
draft = false
title = "libblas 3.12.0-5"
version = "3.12.0-5"
date = "2024-02-12T09:33:51"
aliases = "/packages/219016"
categories = ['lib']
upstreamurl = "http://www.netlib.org/lapack"
arch = "x86_64"
size = "214700"
usize = "677954"
sha1sum = "0f0f162771eca95b134de6e5ad7bbc657101a27f"
depends = "['libgfortran>=11.2']"
reverse depends = "['lapack', 'libcblas', 'libcblas64', 'liblapacke', 'liblapacke64']"
files = "['/usr/lib/libblas.so', '/usr/lib/libblas.so.3', '/usr/lib/libblas.so.3.12.0', '/usr/lib/pkgconfig/blas.pc']"
+++
Basic Linear Algebra Subprograms.