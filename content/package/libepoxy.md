+++
draft = false
title = "libepoxy 1.5.10-2"
version = "1.5.10-2"
description = "Epoxy is a library for handling OpenGL function pointer management for you"
date = "2024-05-28T10:59:23"
aliases = "/packages/201113"
categories = ['x11']
upstreamurl = "https://github.com/anholt/libepoxy"
arch = "x86_64"
size = "297428"
usize = "2918492"
sha1sum = "612eb5309ff75693f3ecf04bd2381f0cb794126e"
depends = "['libglvnd', 'libx11>=1.6.5-2', 'libxdamage>=1.1.4-3', 'libxext>=1.3.3-3', 'libxshmfence>=1.2-3', 'libxxf86vm>=1.1.4-3', 'mesa-libegl']"
reverse_depends = "['blender', 'gtk+3', 'gtk+3-demos', 'gtk+4', 'gtk+4-demos', 'kdeclarative', 'kdeclarative5', 'kpipewire', 'krfb-pipewire', 'libgltf', 'libplacebo', 'performous', 'qemu', 'qt6-webengine', 'virglrenderer', 'wpebackend-fdo', 'xorg-server', 'xorg-server-xephyr', 'xorg-server-xwayland']"
+++
### Description: 
Epoxy is a library for handling OpenGL function pointer management for you

### Files: 
* /usr/include/epoxy/common.h
* /usr/include/epoxy/egl.h
* /usr/include/epoxy/egl_generated.h
* /usr/include/epoxy/gl.h
* /usr/include/epoxy/glx.h
* /usr/include/epoxy/glx_generated.h
* /usr/include/epoxy/gl_generated.h
* /usr/lib/libepoxy.so
* /usr/lib/libepoxy.so.0
* /usr/lib/libepoxy.so.0.0.0
* /usr/lib/pkgconfig/epoxy.pc
* /usr/share/doc/libepoxy-1.5.10/COPYING
* /usr/share/doc/libepoxy-1.5.10/README.md
