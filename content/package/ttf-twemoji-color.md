+++
draft = false
title = "ttf-twemoji-color 14.0.2-1"
version = "14.0.2-1"
description = "A color and B&W emoji SVG-in-OpenType font with support for ZWJ, skin tone modifiers and country flags."
date = "2022-12-19T15:35:50"
aliases = "/packages/219230"
categories = ['fonts-extra']
upstreamurl = "https://github.com/eosrei/twemoji-color-font"
arch = "x86_64"
size = "3571516"
usize = "14491411"
sha1sum = "2d0570718deed76bd7696fca9dcfb414d8e454ad"
depends = "['fontconfig', 'mkfontscale', 'ttf-bitstream-vera']"
+++
A color and B&W emoji SVG-in-OpenType font with support for ZWJ, skin tone modifiers and country flags.{{< files text="show files" >}}* /etc/fonts/conf.avail/46-twemoji-color.conf
* /etc/fonts/conf.d/46-twemoji-color.conf
* /usr/share/doc/ttf-twemoji-color-14.0.2/README.md
* /usr/share/fonts/X11/TTF/TwitterColorEmoji-SVGinOT.ttf
{{< /files >}}