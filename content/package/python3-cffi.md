+++
draft = false
title = "python3-cffi 1.16.0-1"
version = "1.16.0-1"
description = "cryptography expose cryptographic recipes and primitives in a python module"
date = "2023-10-05T19:32:55"
aliases = "/packages/219786"
categories = ['devel-extra']
upstreamurl = "http://pypi.python.org/pypi/cffi"
arch = "x86_64"
size = "261472"
usize = "1004194"
sha1sum = "8945d901a1fd7a61bc8f6b956304e8904fd60a84"
depends = "['libffi>=3.4', 'python3>=3.12', 'python3-pycparser>=2.16']"
reverse_depends = "['lutris', 'python3-bcrypt', 'python3-cryptography', 'python3-xattr']"
+++
cryptography expose cryptographic recipes and primitives in a python module{{< files text="show files" >}}* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/dependency_links.txt
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/entry_points.txt
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/not-zip-safe
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/PKG-INFO
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/requires.txt
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/SOURCES.txt
* /usr/lib/python3.12/site-packages/cffi-1.16.0-py3.12.egg-info/top_level.txt
* /usr/lib/python3.12/site-packages/cffi/api.py
* /usr/lib/python3.12/site-packages/cffi/backend_ctypes.py
* /usr/lib/python3.12/site-packages/cffi/cffi_opcode.py
* /usr/lib/python3.12/site-packages/cffi/commontypes.py
* /usr/lib/python3.12/site-packages/cffi/cparser.py
* /usr/lib/python3.12/site-packages/cffi/error.py
* /usr/lib/python3.12/site-packages/cffi/ffiplatform.py
* /usr/lib/python3.12/site-packages/cffi/lock.py
* /usr/lib/python3.12/site-packages/cffi/model.py
* /usr/lib/python3.12/site-packages/cffi/parse_c_type.h
* /usr/lib/python3.12/site-packages/cffi/pkgconfig.py
* /usr/lib/python3.12/site-packages/cffi/recompiler.py
* /usr/lib/python3.12/site-packages/cffi/setuptools_ext.py
* /usr/lib/python3.12/site-packages/cffi/vengine_cpy.py
* /usr/lib/python3.12/site-packages/cffi/vengine_gen.py
* /usr/lib/python3.12/site-packages/cffi/verifier.py
* /usr/lib/python3.12/site-packages/cffi/_cffi_errors.h
* /usr/lib/python3.12/site-packages/cffi/_cffi_include.h
* /usr/lib/python3.12/site-packages/cffi/_embedding.h
* /usr/lib/python3.12/site-packages/cffi/_imp_emulation.py
* /usr/lib/python3.12/site-packages/cffi/_shimmed_dist_utils.py
* /usr/lib/python3.12/site-packages/cffi/__init__.py
* /usr/lib/python3.12/site-packages/cffi/__pycache__/api.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/backend_ctypes.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/cffi_opcode.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/commontypes.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/cparser.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/error.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/ffiplatform.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/lock.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/model.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/pkgconfig.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/recompiler.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/setuptools_ext.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/vengine_cpy.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/vengine_gen.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/verifier.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/_imp_emulation.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/_shimmed_dist_utils.cpython-312.pyc
* /usr/lib/python3.12/site-packages/cffi/__pycache__/__init__.cpython-312.pyc
* /usr/lib/python3.12/site-packages/_cffi_backend.cpython-312-x86_64-linux-gnu.so
* /usr/share/doc/python3-cffi-1.16.0/AUTHORS
* /usr/share/doc/python3-cffi-1.16.0/COPYING
* /usr/share/doc/python3-cffi-1.16.0/LICENSE
* /usr/share/doc/python3-cffi-1.16.0/README.md
{{< /files >}}