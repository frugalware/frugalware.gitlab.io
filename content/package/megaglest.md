+++
draft = false
title = "megaglest 3.13.0-3"
version = "3.13.0-3"
description = "Fork of Glest, a 3D real-time strategy game in a fantastic world."
date = "2024-11-16T22:31:55"
aliases = "/packages/222511"
categories = ['games-extra']
upstreamurl = "https://github.com/MegaGlest/megaglest-source"
arch = "x86_64"
size = "3386196"
usize = "12690896"
sha1sum = "96c01ccf35c53b3a9d4fb0637131006c3d5fcb0a"
depends = "['curl', 'ftgl', 'glew>=1.13', 'libgl', 'libircclient', 'libvlc', 'libvorbis', 'lua', 'megaglest-data>=$pkgver', 'miniupnpc>=1.9', 'openal', 'sdl2', 'xerces-c>=3.1.1']"
+++
### Description: 
Fork of Glest, a 3D real-time strategy game in a fantastic world.

### Files: 
* /usr/bin/megaglest
* /usr/bin/megaglest_editor
* /usr/bin/megaglest_g3dviewer
* /usr/share/megaglest/editor.ico
* /usr/share/megaglest/g3dviewer.ico
* /usr/share/megaglest/glest.ini
* /usr/share/megaglest/glestkeys.ini
* /usr/share/megaglest/megaglest.ico
* /usr/share/megaglest/servers.ini
* /usr/share/megaglest/start_megaglest_gameserver
