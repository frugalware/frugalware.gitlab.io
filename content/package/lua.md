+++
draft = false
title = "lua 5.4.7-1"
version = "5.4.7-1"
description = "A powerful light-weight programming language designed for extending applications."
date = "2024-06-27T07:16:23"
aliases = "/packages/3219"
categories = ['devel']
upstreamurl = "http://www.lua.org/"
arch = "x86_64"
size = "207460"
usize = "833943"
sha1sum = "9717714591ba71bb638ed005abb1dd923ee31542"
depends = "['glibc>=2.34', 'ncurses>=6.0-18', 'readline>=8.0']"
reverse_depends = "['conky', 'crawl', 'crawl-tiles', 'darktable', 'eiskaltdc', 'geany-plugins', 'haskell-lua', 'imapfilter', 'libquvi', 'libquvi-scripts', 'lighttpd', 'lpeg', 'luajit2', 'luarocks', 'megaglest', 'mod_security', 'pdns', 'pdns-recursor', 'podofo', 'texworks', 'ultrastardx', 'uwsgi-plugin-lua', 'vlc-lua', 'weechat', 'wireplumber', 'wireshark']"
+++
### Description: 
A powerful light-weight programming language designed for extending applications.

### Files: 
* /usr/bin/lua
* /usr/bin/luac
* /usr/include/lauxlib.h
* /usr/include/lua.h
* /usr/include/lua.hpp
* /usr/include/luaconf.h
* /usr/include/lualib.h
* /usr/lib/liblua.so
* /usr/lib/liblua.so.5.4
* /usr/lib/liblua.so.5.4.7
* /usr/lib/pkgconfig/lua-5.4.pc
* /usr/lib/pkgconfig/lua.pc
* /usr/lib/pkgconfig/lua5.4.pc
* /usr/lib/pkgconfig/lua54.pc
* /usr/share/doc/lua-5.4.7/README
* /usr/share/man/man1/lua.1.gz
* /usr/share/man/man1/luac.1.gz
