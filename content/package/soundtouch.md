+++
draft = false
title = "soundtouch 2.3.1-1"
version = "2.3.1-1"
description = "An open-source audio processing library for changing the Tempo, Pitch and Playback Rates of audio streams or files."
date = "2022-01-20T09:59:23"
aliases = "/packages/9620"
categories = ['multimedia']
upstreamurl = "http://www.surina.net/soundtouch/"
arch = "x86_64"
size = "74996"
usize = "245815"
sha1sum = "554bc34b1f068b54039dcb0800518566633221b3"
depends = "['libstdc++>=11.2']"
reverse_depends = "['easyeffects', 'gst1-plugins-bad-soundtouch', 'mixxx']"
+++
An open-source audio processing library for changing the Tempo, Pitch and Playback Rates of audio streams or files.{{< spoiler text="show files" >}}* /usr/bin/soundstretch
* /usr/include/soundtouch/BPMDetect.h
* /usr/include/soundtouch/FIFOSampleBuffer.h
* /usr/include/soundtouch/FIFOSamplePipe.h
* /usr/include/soundtouch/SoundTouch.h
* /usr/include/soundtouch/soundtouch_config.h
* /usr/include/soundtouch/STTypes.h
* /usr/lib/libSoundTouch.so
* /usr/lib/libSoundTouch.so.1
* /usr/lib/libSoundTouch.so.1.0.0
* /usr/lib/pkgconfig/soundtouch.pc
* /usr/share/aclocal/soundtouch.m4
* /usr/share/doc/soundtouch-2.3.1/COPYING.TXT
* /usr/share/doc/soundtouch-2.3.1/README.html
{{< /spoiler >}}