+++
draft = false
title = "sshfs 3.7.3-3"
version = "3.7.3-3"
description = "A filesystem client based on the SSH File Transfer Protocol."
date = "2024-01-30T10:10:43"
aliases = "/packages/218974"
categories = ['apps-extra']
upstreamurl = "https://github.com/libfuse/sshfs"
arch = "x86_64"
size = "45308"
usize = "112039"
sha1sum = "17a97958d022ee917472079b7b99e6e6e4f9ed1f"
depends = "['fuse3>=3.4.1', 'glib2']"
+++
A filesystem client based on the SSH File Transfer Protocol.{{< spoiler text="show files" >}}* /usr/bin/mount.fuse.sshfs
* /usr/bin/mount.sshfs
* /usr/bin/sshfs
* /usr/share/doc/sshfs-3.7.3/AUTHORS
* /usr/share/doc/sshfs-3.7.3/COPYING
* /usr/share/doc/sshfs-3.7.3/README.rst
* /usr/share/man/man1/sshfs.1.gz
{{< /spoiler >}}