+++
draft = false
title = "rust-bindgen 0.69.1-1"
version = "0.69.1-1"
description = "Automatically generates Rust FFI bindings to C (and some C++) libraries"
date = "2023-12-14T15:25:20"
aliases = "/packages/220819"
categories = ['devel-extra']
upstreamurl = "https://github.com/rust-lang/rust-bindgen"
arch = "x86_64"
size = "1803492"
usize = "7163018"
sha1sum = "b4df8f9ba0679d116217796f6a198a2544b79805"
depends = "['libgcc>=12.2']"
+++
Automatically generates Rust FFI bindings to C (and some C++) libraries{{< spoiler text="show files" >}}* /usr/bin/bindgen
* /usr/share/doc/rust-bindgen-0.69.1/LICENSE
* /usr/share/doc/rust-bindgen-0.69.1/README.md
{{< /spoiler >}}