+++
draft = false
title = "pptpd 1.4.0-4"
version = "1.4.0-4"
description = "Poptop is the PPTP server solution for Linux."
date = "2024-01-15T15:35:12"
aliases = "/packages/10611"
categories = ['network-extra']
upstreamurl = "http://www.poptop.org/"
arch = "x86_64"
size = "59712"
usize = "189975"
sha1sum = "ef33ddc4ae6253dc6fad7e2e4de2d163fe2f1194"
depends = "['ppp>=2.5.0']"
+++
Poptop is the PPTP server solution for Linux.{{< spoiler text="show files" >}}* /etc/ppp/options.pptpd
* /etc/pptpd.conf
* /usr/bin/bcrelay
* /usr/bin/pptpctrl
* /usr/bin/pptpd
* /usr/lib/pptpd/pptpd-logwtmp.so
* /usr/share/doc/pptpd-1.4.0/AUTHORS
* /usr/share/doc/pptpd-1.4.0/ChangeLog
* /usr/share/doc/pptpd-1.4.0/COPYING
* /usr/share/doc/pptpd-1.4.0/INSTALL
* /usr/share/doc/pptpd-1.4.0/NEWS
* /usr/share/doc/pptpd-1.4.0/README
* /usr/share/doc/pptpd-1.4.0/README.bcrelay
* /usr/share/doc/pptpd-1.4.0/README.cvs
* /usr/share/doc/pptpd-1.4.0/README.Frugalware
* /usr/share/doc/pptpd-1.4.0/README.inetd
* /usr/share/doc/pptpd-1.4.0/README.logwtmp
* /usr/share/doc/pptpd-1.4.0/README.portslave
* /usr/share/doc/pptpd-1.4.0/README.slirp
* /usr/share/doc/pptpd-1.4.0/TODO
* /usr/share/man/man5/pptpd.conf.5.gz
* /usr/share/man/man8/bcrelay.8.gz
* /usr/share/man/man8/pptpctrl.8.gz
* /usr/share/man/man8/pptpd.8.gz
{{< /spoiler >}}